...
class DroneRoutingDataset(Dataset):
    """Enhanced dataset for GNN training with better feature engineering"""
    ...
    def __init__(self, training_data: List[Dict]):
        ...
        for data_sample in training_data:
            ...
            for i, drone_candidate in enumerate(data_sample['state']['drone_candidates']):
                # Create input features combining order, system, and drone features
                input_features = np.concatenate([
                    state_features,
                    self._extract_drone_features(drone_candidate)
                ])
                
                # Label: 1 if this drone was chosen, 0 otherwise
                label = 1.0 if drone_candidate['drone_id'] == chosen_drone_id else 0.0
        ...
        print(f"Created dataset with {len(self.samples)} samples, feature dim: {self.samples.shape[1]}")
    
    def _extract_enhanced_state_features(self, state: Dict) -> np.ndarray:
        """Extract enhanced state features for better decision making"""
        features = []
        ...
        features.extend([
            ...
            order['urgency_ratio'],
            order['order_distance'] / 1000,  # Convert to km
        ])
        ...
        if drone_candidates:
            ...
            avg_battery = min_distance = max_distance = num_candidates = 0
        
        features.extend([avg_battery, min_distance/1000, max_distance/1000, num_candidates])
        
        return np.array(features)
    
    def _extract_drone_features(self, drone_candidate: Dict) -> np.ndarray:
        """Extract individual drone features"""
        return np.array([
            drone_candidate['battery_level'],
            drone_candidate['distance_to_pickup'] / 1000,  # Convert to km
    ...
    def __getitem__(self, idx):
        return torch.FloatTensor(self.samples[idx]), torch.FloatTensor([self.labels[idx]])

class ImprovedDroneAssignmentGNN(nn.Module):
    """Enhanced GNN for drone assignment with attention mechanisms"""
    
    def __init__(self, input_dim, hidden_dim=256, num_layers=4, dropout=0.2):
        super().__init__()
        ...
        # Score prediction head with residual connections
        ...
        # Future horizon predictor for better long-term planning
        self.future_value_head = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x_seq = x_embedded.unsqueeze(1)  # (batch, 1, hidden_dim)
        ...
        x_final = x_seq.squeeze(1)
        
        # Predict assignment score and future value
        assignment_score = self.score_predictor(x_final)
        future_value = self.future_value_head(x_final)
        
        # Combine current assignment score with future value estimation
        combined_score = assignment_score + 0.3 * future_value  # 30% future value weight
        
        return torch.sigmoid(combined_score.squeeze(-1))

class GNNDroneRoutingOptimizer:
    """GNN-based drone routing optimizer with enhanced assignment strategy"""
    ...
    def _load_trained_model(self):
        ...
        try:
            ...
            input_dim = checkpoint.get('input_dim', 26)  # Default based on feature extraction
            self.model = ImprovedDroneAssignmentGNN(input_dim)
    ...
    def calculate_total_mission_time(self, drone: Drone, order: Order) -> float:
        """Calculate total mission time including service times (returns seconds)"""
        ...
        delivery_to_depot_time = self.calculate_flight_time(order.delivery_location, drone.location)
    ...
    def can_complete_order(self, drone: Drone, order: Order) -> bool:
        """Check if drone can complete order within all constraints"""
        ...
        mission_time = self.calculate_total_mission_time(drone, order)
    ...
    def _create_state_features(self, order: Order, available_drones: List[Drone]) -> Dict:
        """Create enhanced state representation for GNN"""
        # Order features
        order_features = {
        ...
        # Drone candidates features
        drone_candidates = []
        for drone in available_drones:
            mission_time = self.calculate_total_mission_time(drone, order)
            distance_to_pickup = self.haversine_distance(drone.location, order.pickup_location)
            distance_delivery_to_depot = self.haversine_distance(order.delivery_location, drone.location)
            order_distance = self.haversine_distance(order.pickup_location, order.delivery_location)
    ...
    def assign_orders_gnn(self):
        """GNN-based assignment with enhanced scoring"""
        assignments = []
        ...
        for order in self.pending_orders[:]:
            ...
            if self.model is not None and len(available_drones) > 0:
                # Use GNN for assignment
                try:
                    state_features = self._create_state_features(order, available_drones)
                    
                    # Extract state features for all drone candidates
                    state_base = self._extract_state_features_for_gnn(state_features)
                    
                    drone_scores = []
                    for drone in available_drones:
                        # Create input for this drone
                        ...
                        # Combine state and drone features
                        input_features = np.concatenate([state_base, drone_features])
                    ...
                    # Select drone with highest GNN score
                    if drone_scores:
                        best_drone, best_score = max(drone_scores, key=lambda x: x[1])
                        self.gnn_predictions.append(best_score)
                
                except Exception as e:
                    print(f"GNN prediction failed: {e}, falling back to heuristic")
                    best_drone = None
            
            # Fallback to enhanced heuristic if GNN fails or not available
            if best_drone is None:
                for drone in available_drones:
                    ...
                    total_distance = (
                        ...
                        self.haversine_distance(order.delivery_location, drone.location)
                    )
                    distance_penalty = total_distance / 100000
                    ...
                    if len(self.pending_orders) > 5:  # If many orders pending
                        future_penalty = -1  # Small penalty for not reserving capacity
                    
                    # Final score calculation
                    assignment_score = ((base_score + time_bonus + battery_bonus - distance_penalty + future_penalty) 
                                      * urgency_multiplier)
    ...
    def _extract_state_features_for_gnn(self, state_features: Dict) -> np.ndarray:
        """Extract state features in same format as training data"""
        features = []
    ...
    def _extract_drone_features_for_gnn(self, drone_candidate: Dict) -> np.ndarray:
        """Extract individual drone features for GNN"""
        return np.array([
            drone_candidate['battery_level'],
    ...
    def update_drone_positions(self, dt: float):
        """Update drone positions and handle state transitions"""
            ...
            if move_distance_meters >= distance_to_target:
                ...
                if drone.status == DroneStatus.FLYING_TO_PICKUP and drone.current_order:
                    ...
                    if pickup_distance < 50:
                        ...
                        drone.full_route = [drone.current_order.delivery_location, 
                    ...
                    if delivery_distance < 50:
                        ...
                        # Return to depot
                        depot_location = next(d.location for d in self.depots if d.id == drone.depot_id)
                        drone.route = [depot_location]
                        drone.full_route = [depot_location]
                    ...
                    depot_location = next(d.location for d in self.depots if d.id == drone.depot_id)
                    depot_distance = self.haversine_distance(drone.location, depot_location)
    ...
    def execute_assignments(self, assignments: List[Tuple[Drone, Order]]):
        ...
        for drone, order in assignments:
            ...
            drone.full_route = [order.pickup_location, order.delivery_location,
                              next(d.location for d in self.depots if d.id == drone.depot_id)]
    ...
    def step(self, dt: float, new_orders: List[Order] = None):
        ...
        # Use GNN-based assignment
        assignments = self.assign_orders_gnn()
    ...
    def print_final_report(self):
        ...
        print("\n" + "="*80)
        print("GNN-BASED DRONE ROUTING PERFORMANCE REPORT")
        print("="*80)
        ...
        print(f"Model Used: {'Trained GNN' if self.model else 'Enhanced Heuristic'}")
        ...
        if self.gnn_predictions:
            print(f"\nGNN PERFORMANCE:")
            print(f"  Average GNN Score: {np.mean(self.gnn_predictions):.3f}")
            print(f"  GNN Predictions Made: {len(self.gnn_predictions)}")

class GNNDroneRoutingVisualizer:
    """Enhanced visualizer for GNN-based drone routing"""
    
    def __init__(self, optimizer: GNNDroneRoutingOptimizer, bounds: Tuple[Tuple[float, float], Tuple[float, float]]):
        ...
        self.ax_main.set_title('GNN-Enhanced Drone Routing System')
        self.ax_main.grid(True, alpha=0.3)
        
        # Metrics plots setup
        self.ax_score.set_title('Score Performance')
        self.ax_completion.set_title('Order Completion')
        self.ax_utilization.set_title('Drone Utilization')
    ...
    def update_visualization(self, frame):
        ...
        model_status = "GNN" if self.optimizer.model else "Heuristic"
        
        self.ax_main.set_title(f'GNN-Enhanced Drone Routing ({model_status}) - t={self.optimizer.current_time:.1f}min - Score: {current_score}/{max_score} ({score_pct:.1f}%)')
        self.ax_main.grid(True, alpha=0.3)
        ...
        for i, depot in enumerate(self.optimizer.depots):
            ...
            self.ax_main.annotate(f'D{depot.id}', depot.location, 
        ...
        # Plot drones with enhanced status indicators
        ...
        self.ax_utilization.set_ylabel('Utilization Percentage')
        self.ax_utilization.set_xlabel('Time (minutes)')
        self.ax_utilization.set_title('Drone Utilization')
        self.ax_utilization.grid(True, alpha=0.3)
        self.ax_utilization.legend()

def load_training_data_from_folder(folder_path: str) -> List[Dict]:
    """Load training data from multiple files in gnn_training_data folder"""
    training_data = []
    
    if not os.path.exists(folder_path):
        print(f"Training data folder {folder_path} not found!")
        return training_data
    
    pkl_files = glob.glob(os.path.join(folder_path, "*_sol.pkl"))
    ...
    for file_path in pkl_files:
        try:
            ...
            if 'assignment_decisions' in data:
                training_data.extend(data['assignment_decisions'])
...
def train_gnn_model(epochs=200, batch_size=64, learning_rate=0.001):
    """Train the GNN model on selected training instances"""
    
    print("="*80)
    print("TRAINING GNN MODEL FOR DRONE ASSIGNMENT")
    print("="*80)
    ...
    # Create dataset
    dataset = DroneRoutingDataset(all_training_data)
    ...
    model = ImprovedDroneAssignmentGNN(input_dim)
    ...
    print("Model saved as 'best_gnn_drone_model.pth'")
    
    return model, dataset.scaler

def run_gnn_simulation(instance_file: str, dt: float = 0.5, save_metrics: bool = True, show_animation: bool = True):
    """Run simulation with GNN-based optimizer"""
    ...
    model_path = 'best_gnn_drone_model.pth'
    optimizer = GNNDroneRoutingOptimizer(instance, model_path)
    
    # Initialize visualizer
    if show_animation:
        visualizer = GNNDroneRoutingVisualizer(optimizer, instance['bounds'])
...
def run_gnn_batch_experiments():
    """Run GNN experiments on all instances"""
    ...
    print(f"Found {len(instance_files)} instances to process with GNN")
    
    # Check if trained model exists
    model_path = 'best_gnn_drone_model.pth'
    if not os.path.exists(model_path):
        print(f"Trained model {model_path} not found!")
        print("Please run training first: python gnn_new.py train")
        return
    ...
    for instance_file in instance_files:
        ...
        try:
            ...
            optimizer = run_gnn_simulation(instance_file, dt=0.5, 
            ...
            summary_data = {
                ...
                'model_used': 'GNN' if optimizer.model else 'Heuristic',
    ...
    if summary_results:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = f"GNN_results/gnn_batch_summary_{timestamp}.csv"
        ...
        print(f"\n{'='*80}")
        print(f"GNN BATCH PROCESSING COMPLETE")
        print(f"{'='*80}")
...
def main():
    """Main function with three operation modes"""
    print("GNN-Enhanced Drone Routing System")
    print("=" * 80)
    print("Enhanced GNN model focusing on score optimization and future horizon planning")
    print()
    
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python gnn_new.py [train|sample|batch]")
        print()
        print("Options:")
        print("  train  - Train GNN model on selected instances (12 instances total)")
        print("  sample - Run sample instance with trained GNN model")
        print("  batch  - Run all instances with trained GNN model")
        return
    
    mode = sys.argv[1].lower()
    
    if mode == "train":
        print("TRAINING MODE: Training GNN on selected instances")
        print("-" * 50)
        ...
        if not training_files:
            ...
            print("Please run greedy approach first to generate training data.")
            return
        
        print(f"Found {len(training_files)} training instances")
        
        # Train model
        model, scaler = train_gnn_model(epochs=200, batch_size=64, learning_rate=0.001)
        ...
        print("SAMPLE MODE: Running sample instance with trained GNN")
        print("-" * 50)
        
        # Check if trained model exists
        if not os.path.exists('best_gnn_drone_model.pth'):
            print("Trained model not found! Please run training first:")
            print("python gnn_new.py train")
            return
        ...
        if sample_file:
            print(f"Running sample simulation with {sample_file}")
            optimizer = run_gnn_simulation(sample_file, dt=0.5, show_animation=True)
        else:
            print("No sample instance found!")
            print("Expected instances in 'instances/' folder")
    
    elif mode == "batch":
        print("BATCH MODE: Running all instances with trained GNN")
        print("-" * 50)
