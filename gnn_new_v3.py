...
class CrossDepotAwareGNN(nn.Module):
    """Advanced GNN with learnable scoring coefficients"""
    ...
    def __init__(self, input_dim, hidden_dim=512, num_depots=5, dropout=0.3):
        ...
        # CHANGE 1: Replace multiple prediction heads with unified scoring network
        self.unified_scorer = nn.Sequential(
            nn.Linear(hidden_dim // 2 + 32 + 64, 256),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(256, 128),
            nn.ReLU(),
            ...
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 1)  # Single output for final assignment score
        )
        
        # CHANGE 2: Learnable coefficients for multi-task components (when needed for training)
        self.assignment_head = nn.Sequential(
            nn.Linear(hidden_dim // 2 + 32 + 64, 64),
            nn.ReLU(),
            nn.Linear(64, 1),
            nn.Sigmoid()
        )
        
        self.cross_depot_head = nn.Sequential(
            nn.Linear(hidden_dim // 2 + 32, 32),
            nn.ReLU(),
            nn.Linear(32, 1),
            nn.Sigmoid()
        )
        ...
        # CHANGE 3: Learnable weight coefficients instead of fixed ones
        self.scoring_weights = nn.Parameter(torch.tensor([0.4, 0.3, 0.2, 0.1]))  # Learnable weights
        
        # CHANGE 4: Temperature parameter for score scaling
        self.temperature = nn.Parameter(torch.tensor(1.0))
        
    def forward(self, x, depot_ids=None, training=False):
        batch_size = x.size(0)
        ...
        if training:
            # CHANGE 5: During training, use separate heads for multi-task learning
            assignment_prob = self.assignment_head(assignment_features)
            cross_depot_prob = self.cross_depot_head(combined_features)
            depot_preferences = self.depot_selector(combined_features)
            future_value = self.value_estimator(combined_features)
            
            # Learnable weighted combination for training loss
            normalized_weights = F.softmax(self.scoring_weights, dim=0)
            enhanced_score = (
                assignment_prob.squeeze(-1) * normalized_weights[0] +
                cross_depot_prob.squeeze(-1) * normalized_weights[1] +
                future_value.squeeze(-1) * normalized_weights[2] +
                depot_preferences.max(dim=1)[0] * normalized_weights[3]
            )
            
            return {
                'assignment_score': torch.sigmoid(enhanced_score / self.temperature),
                ...
                'learned_weights': normalized_weights  # For monitoring
            }
        else:
            # CHANGE 6: During inference, use unified scorer for direct utility prediction
            unified_score = self.unified_scorer(assignment_features)
            
            # Also compute individual components for monitoring/debugging
            assignment_prob = self.assignment_head(assignment_features)
            cross_depot_prob = self.cross_depot_head(combined_features)
            depot_preferences = self.depot_selector(combined_features)
            future_value = self.value_estimator(combined_features)
            ...
            return {
                'assignment_score': torch.sigmoid(unified_score.squeeze(-1) / self.temperature),
                'cross_depot_probability': cross_depot_prob.squeeze(-1),
                'depot_preferences': depot_preferences,
                'future_value': future_value.squeeze(-1),
                'raw_assignment_prob': assignment_prob.squeeze(-1),
                'learned_weights': F.softmax(self.scoring_weights, dim=0)  # For monitoring
            }
                ...
                try:
                    ...
                    for drone in available_drones:
                        ...
                        with torch.no_grad():
                            ...
                            predictions = self.model(input_tensor, depot_id_tensor, training=False)
...
def train_advanced_gnn_model(epochs=300, batch_size=64, learning_rate=0.001):
    ...
    for epoch in range(epochs):
        ...
        for batch_features, batch_labels, batch_depot_info in train_loader:
            ...
            # CHANGE: Pass training=True flag
            predictions = model(batch_features, depot_ids, training=True)
            ...
            # CHANGE: Add unified score loss
            unified_loss = assignment_criterion(predictions['assignment_score'], batch_labels.squeeze())
            
            total_loss = assignment_loss + cross_depot_loss + value_loss + unified_loss
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'best_advanced_gnn_model_v2.pth')
        else:
            patience_counter += 1
    ...
    print("Model saved as 'best_advanced_gnn_model_v2.pth'")
...
def run_advanced_gnn_simulation(instance_file: str, dt: float = 0.5, save_metrics: bool = True, show_animation: bool = True):
    ...
    model_path = 'best_advanced_gnn_model_v2.pth'
...
def main():
    ...
    if mode == "train":
        ...
        # Check if trained model exists
        if not os.path.exists('best_advanced_gnn_model_v2.pth'):
