...
class ImprovedDroneRoutingOptimizer:
    """Improved optimizer with real-world drone constraints and scoring system"""
    ...
    def __init__(self, instance: Dict):
        ...
        # Removed metrics_history - no longer needed
    ...
    def collect_gnn_training_data(self, order: Order, available_drones: List[Drone], 
                                chosen_drone: Drone, assignment_time: float):
        """Collect enhanced training data for GNN model with better objectives"""
        ...
        # Action taken - Which drone was chosen
        action = {
        ...
        reward_components = self._calculate_comprehensive_reward(order, chosen_drone, assignment_time)
    ...
    def _calculate_comprehensive_reward(self, order: Order, drone: Drone, assignment_time: float) -> Dict:
        """Calculate comprehensive reward components for GNN training"""
        mission_time = self.calculate_total_mission_time(drone, order)
        ...
        assignment_speed_reward = max(0, 2 - assignment_time)  # Reward fast decisions
        ...
        rewards = {
            ...
            'total_reward': (score_reward + time_reward + resource_reward - distance_penalty) * urgency_multiplier + assignment_speed_reward,
            'normalized_total_reward': ((score_reward + time_reward + resource_reward - distance_penalty) * urgency_multiplier + assignment_speed_reward) / 20  # Normalize to [0,1] range approximately
        }
        
        return rewards
    
    def assign_orders_greedy(self):
        """Enhanced greedy assignment focusing on score maximization with timing"""
        assignments = []
        ...
        for order in self.pending_orders[:]:
            ...
            for drone in available_drones:
                ...
                # Base score from order priority
                base_score = order.score
                
                # Time efficiency bonus (faster completion = higher score)
                completion_time = self.current_time + mission_time / 60
                time_efficiency = max(0, (order.deadline - completion_time)) / order.deadline
                time_bonus = time_efficiency * 2
                
                # Distance penalty (shorter missions preferred)
                total_distance = (
                ...
                battery_bonus = battery_efficiency * 1
                
                # Urgency multiplier for high priority orders
                urgency_multiplier = 1.0
                time_until_deadline = (order.deadline - self.current_time) / 60  # minutes to hours
                if order.priority == OrderPriority.EMERGENCY and time_until_deadline < 1:
                    urgency_multiplier = 2.0
                elif order.priority == OrderPriority.HIGH and time_until_deadline < 2:
                    urgency_multiplier = 1.5
                
                # Final score calculation
                assignment_score = (base_score + time_bonus + battery_bonus - distance_penalty) * urgency_multiplier
            ...
            if best_drone:
                ...
                # Collect GNN training data
                self.collect_gnn_training_data(order, available_drones, best_drone, assignment_duration)
        ...
        for drone in self.drones:
            ...
            if move_distance_meters >= distance_to_target:
                ...
                if drone.status == DroneStatus.FLYING_TO_PICKUP and drone.current_order:
                    ...
                    if depot_distance < 50:  # Within 50m of depot
                        drone.status = DroneStatus.IDLE
                        drone.full_route = []
    ...
    def execute_assignments(self, assignments: List[Tuple[Drone, Order]]):
        ...
        for drone, order in assignments:
            ...
            print(f"Drone {drone.id} assigned to order {order.id} at time {self.current_time:.1f}")
            print(f"  From depot at {drone.location} to pickup at {order.pickup_location}")
            print(f"  Battery level: {drone.current_battery_seconds/drone.battery_duration_seconds*100:.1f}%")
    ...
    def _record_metrics_snapshot(self):
        """Record current metrics for CSV export - REMOVED FOR SINGLE ROW OUTPUT"""
        # This method is no longer used - we save final metrics only
        pass
    ...
    def save_metrics_to_csv(self, filename: str):
        ...
        # Create single row with all final metrics
        final_metrics = {
    ...
    def print_final_report(self):
        ...
        for drone in self.drones:
            ...
            print(f"  Drone {drone.id}: Distance={drone.total_distance_meters/1000:.1f}km, "
...
class ImprovedDroneRoutingVisualizer:
    """Enhanced visualizer with better performance metrics display"""
    ...
    def __init__(self, optimizer: ImprovedDroneRoutingOptimizer, bounds: Tuple[Tuple[float, float], Tuple[float, float]]):
        ...
        self.ax_main.set_title('Enhanced Drone Routing System')
    ...
    def update_visualization(self, frame):
        ...
        self.ax_main.set_title(f'Enhanced Drone Routing (t={self.optimizer.current_time:.1f}min) - Score: {current_score}/{max_score} ({score_pct:.1f}%)')
        self.ax_main.grid(True, alpha=0.3)
        ...
        for drone in self.optimizer.drones:
            ...
            # Status-based markers and sizes
            status_config = {
...
def run_improved_simulation(instance_file: str, dt: float = 1.0, save_metrics: bool = True, show_animation: bool = True):
    """Run simulation with improved optimizer"""
    ...
    if save_metrics:
        ...
        metrics_file = f"results/metrics_{instance_name}_{timestamp}.csv"
...
def run_batch_experiments():
    ...
    if summary_results:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = f"results/batch_summary_{timestamp}.csv"
        ...
        print(f"Individual metrics saved to: results/")
        print(f"GNN training data saved to: gnn_training_data/")


def main():
    """Main function to run improved greedy approach"""
    print("Enhanced Drone Routing System with Real-World Constraints")
    print("=" * 80)
    
    # Check for sample instance
    sample_instance = "sample_medium_instance.pkl"
    ...
    if os.path.exists(sample_instance):
        ...
        if choice == "1":
            print(f"Running simulation with {sample_instance}")
